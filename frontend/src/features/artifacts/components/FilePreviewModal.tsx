...
// Lazy-load annotation panel (only needed for PPTX files)
const PptxAnnotationPanel = dynamic(() => import('./PptxAnnotationPanel'), {
  ssr: false,
});

/** Check if a file is a PPTX presentation by MIME type or extension. */
...
export interface FilePreviewModalProps {
  ...
  mimeType: string;
  signedUrl: string;
  /** Required for PPTX annotation panel. */
  workspaceId?: string;
  /** Required for PPTX annotation panel. */
  projectId?: string;
}
...
export function FilePreviewModal({
  ...
  const [annotPanelOpen, setAnnotPanelOpen] = React.useState(true);
  ...
  const isPptx = isPptxFile(mimeType, filename);
      ...
      >
        ...
        <DialogHeader className="flex-row items-center justify-between px-4 py-3 border-b shrink-0 gap-2">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {/* Annotation panel toggle (PPTX only) */}
            {isPptx && workspaceId && projectId && (
              <Button
                variant="ghost"
                size="icon"
                className="size-8"
                onClick={() => setAnnotPanelOpen((v) => !v)}
        ...
        <div className="flex-1 overflow-auto min-h-0 flex">
          ...
          {/* Annotation panel for PPTX files */}
          {isPptx && workspaceId && projectId && (
            <React.Suspense fallback={null}>
              <PptxAnnotationPanel
                ...
                isCollapsed={!annotPanelOpen}
                onToggleCollapse={() => setAnnotPanelOpen((v) => !v)}
              />
            </React.Suspense>
          )}
        </div>
