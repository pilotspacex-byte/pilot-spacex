 ...
 *
 * MobX observer that reads current branch from GitStore.
 * Supports title, body, base branch selector, head (read-only), and draft checkbox.
 */
export const CreatePRForm = observer(function CreatePRForm({
  workspaceId,
  owner,
  repo,
  onClose,
  ...
  const { branches } = useBranches(workspaceId, owner, repo);
  const { createPR, isCreating } = useCreatePR(workspaceId, owner, repo);

  const [title, setTitle] = useState('');
  const [body, setBody] = useState('');
  const [baseBranch, setBaseBranch] = useState(gitStore.defaultBranch ?? 'main');
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!canSubmit || !gitStore.currentBranch) return;

    createPR(
      {
        title: title.trim(),
        body,
        head: gitStore.currentBranch,
        base: baseBranch,
        draft,
      },
      {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 px-2 py-2">
      ...
      {/* Head branch (read-only: current branch) */}
      <div className="flex items-center gap-2">
        <Label className="text-xs text-muted-foreground shrink-0">Head:</Label>
        <span className="text-xs font-mono truncate">{gitStore.currentBranch ?? '—'}</span>
      </div>
