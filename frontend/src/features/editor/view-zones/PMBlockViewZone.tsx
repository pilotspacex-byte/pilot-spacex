...
import type { ExtendedPMBlockType, PMBlockType } from '../types';
...
export function PMBlockViewZone({ type, data, raw }: PMBlockViewZoneProps): JSX.Element {
  ...
  const Renderer = rendererMap[type as PMBlockType];
  const label =
    BLOCK_TYPE_LABELS[type as PMBlockType] ??
    getRegisteredBlockTypes().find((bt) => bt.type === type)?.label ??
    type;
