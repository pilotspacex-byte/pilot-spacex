/**
 * Sidebar — Surface 1 unit tests.
 *
 * Verifies the top-stack order (workspace-pill → home → search; no
 * +New chat — Home is the new-chat entry now), the ⌘K Search button
 * → openCommandPalette wiring, the RECENT CHATS empty-state copy, and
 * the WORKSPACE accordion canonical entry order with Phase 84 routes
 * (/tasks, /topics — NOT /issues, /notes).
 *
...
class TestWorkspaceStore {
  ...
  // WorkspaceSwitcher reads workspaceStore.members to resolve the current
  // user's memberId for the Leave flow.
  members = new Map<string, Array<{ id: string; userId: string; role: string }>>([
    ['id-alpha', [{ id: 'mem-a', userId: 'u1', role: 'OWNER' }]],
    ['id-beta', [{ id: 'mem-b', userId: 'u1', role: 'MEMBER' }]],
  ]);
...
class TestAuthStore {
  user = {
    ...
    workspaceMemberships: [
      { workspaceId: 'id-alpha', role: 'OWNER' },
      { workspaceId: 'id-beta', role: 'MEMBER' },
    ],
  };
...
describe('Sidebar v3 (Surface 1 — Plan 90-04)', () => {
  ...
  it('renders the rail filling its parent container on the sidebar surface', () => {
    // Width is now driven by the app-shell parent (240px expanded / 60px
    // collapsed). The aside fills the container with `w-full` and uses the
    // sidebar token surface.
    renderSidebar();
    const rail = screen.getByTestId('sidebar');
    expect(rail.className).toContain('w-full');
    expect(rail.className).toContain('bg-sidebar');
    expect(rail.className).toContain('flex-col');
  });

  it('top stack is rendered in DOM order: workspace-pill → home-button → sidebar-search-button (no New chat button)', () => {
    renderSidebar();
    ...
    expect(pill.compareDocumentPosition(home) & Node.DOCUMENT_POSITION_FOLLOWING).toBe(
      Node.DOCUMENT_POSITION_FOLLOWING
    );
    expect(home.compareDocumentPosition(search) & Node.DOCUMENT_POSITION_FOLLOWING).toBe(
      Node.DOCUMENT_POSITION_FOLLOWING
    );
  });
