...
Tests template-based user skill creation with template-based personalization,
...
class TestCreateUserSkill:
    ...
    async def test_creates_skill_from_template(self, service: CreateUserSkillService) -> None:
        """Creates a user skill with template-based content personalization."""
        user_id = uuid4()
        workspace_id = uuid4()
        template_id = uuid4()
        ...
        with (
            ...
            # Template content + experience context
            assert "# Developer Template" in call_kwargs["skill_content"]
            assert "10 years Python experience" in call_kwargs["skill_content"]
    ...
    async def test_skips_generation_when_skill_content_provided_with_template(
        self, service: CreateUserSkillService
    ) -> None:
        """Uses provided skill_content instead of template-based generation when both template_id and skill_content are given."""
        user_id = uuid4()
        workspace_id = uuid4()
        template_id = uuid4()
