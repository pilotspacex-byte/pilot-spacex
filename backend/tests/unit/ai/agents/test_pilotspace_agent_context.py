"""Unit tests for PilotSpaceAgent._build_contextual_message().
...
Reference: backend/src/pilot_space/ai/agents/pilotspace_agent.py
"""
...
@pytest.fixture
def agent() -> PilotSpaceAgent:
    """Create PilotSpaceAgent instance without dependencies.

    Uses object.__new__ to skip __init__ since we only need to call
    _build_contextual_message which doesn't require initialized state.
    """
    return object.__new__(PilotSpaceAgent)
...
class TestBuildContextualMessage:
    """Test suite for _build_contextual_message method."""

    def test_no_context_returns_original_message(self, agent: PilotSpaceAgent) -> None:
        """Empty context dict should return raw message unchanged."""
        # Arrange
        input_data = ChatInput(message="What is the weather?", context={})

        # Act
        result = agent._build_contextual_message(input_data)

        # Assert
        assert result == "What is the weather?"

    def test_note_context_prepends_markdown(
        self, agent: PilotSpaceAgent, tiptap_content: dict[str, Any]
    ) -> None:
    ...
    def test_selected_text_wrapped(self, agent: PilotSpaceAgent) -> None:
    ...
    def test_empty_note_content_skipped(self, agent: PilotSpaceAgent) -> None:
        """Note with empty content dict should not prepend any context."""
        # Arrange
    ...
    def test_note_with_complex_markdown_content(self, agent: PilotSpaceAgent) -> None:
    ...
    def test_whitespace_only_markdown_skipped(self, agent: PilotSpaceAgent) -> None:
    ...
    def test_note_missing_content_attribute(self, agent: PilotSpaceAgent) -> None:
        """Note object without content attribute should not crash."""
        # Arrange
        ...
        assert result == "Handle this gracefully"
        assert "<note_context>" not in result

    def test_selected_text_preserves_newlines(self, agent: PilotSpaceAgent) -> None:
    ...
    def test_empty_selected_text_not_wrapped(self, agent: PilotSpaceAgent) -> None:
    ...
    def test_none_selected_text_skipped(self, agent: PilotSpaceAgent) -> None:
    ...
    def test_note_with_code_block(self, agent: PilotSpaceAgent) -> None:
