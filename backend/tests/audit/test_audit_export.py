...
class TestAuditExportCsv:
    ...
    @pytest.mark.xfail(strict=False, reason="audit export endpoint implementation pending")
    @pytest.mark.asyncio
    async def test_csv_export_content_type(self, client) -> None:
        """GET /audit/export?format=csv must return text/csv content-type."""
        response = await client.get("/api/v1/workspaces/test-workspace/audit/export?format=csv")
        assert response.status_code == 200
        assert "text/csv" in response.headers.get("content-type", "")

    @pytest.mark.xfail(strict=False, reason="audit export endpoint implementation pending")
    @pytest.mark.asyncio
    async def test_csv_export_has_correct_headers(self, client) -> None:
    ...
    @pytest.mark.xfail(strict=False, reason="audit export endpoint implementation pending")
    @pytest.mark.asyncio
    async def test_csv_export_has_content_disposition_header(self, client) -> None:
...
class TestAuditExportJson:
    ...
    @pytest.mark.xfail(strict=False, reason="audit export endpoint implementation pending")
    @pytest.mark.asyncio
    async def test_json_export_content_type(self, client) -> None:
        """GET /audit/export?format=json must return application/json content-type."""
        response = await client.get("/api/v1/workspaces/test-workspace/audit/export?format=json")
        assert response.status_code == 200
        assert "application/json" in response.headers.get("content-type", "")

    @pytest.mark.xfail(strict=False, reason="audit export endpoint implementation pending")
    @pytest.mark.asyncio
    async def test_json_export_is_array(self, client) -> None:
    ...
    @pytest.mark.xfail(strict=False, reason="audit export endpoint implementation pending")
    @pytest.mark.asyncio
    async def test_json_export_entry_has_required_fields(self, client) -> None:
...
class TestAuditExportFilters:
    ...
    @pytest.mark.xfail(strict=False, reason="audit export endpoint implementation pending")
    @pytest.mark.asyncio
    async def test_export_respects_action_filter(self, client) -> None:
