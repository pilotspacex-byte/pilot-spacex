...
1. **Pre-Tool Hooks (PreToolUse)**
   - Pattern-based tool matching (glob syntax)
   - Command execution for validation scripts
   - Deny patterns for blocking dangerous operations
   - Explicit approval requirements
   - Input modification/injection
...
```json
{
  ...
  "hooks": {
    "PreToolUse": [
      ...
      {
        "matcher": "Write|Edit",
        "description": "Check sensitive file protection",
        "hooks": [
          {
            "type": "deny",
          ...
          },
          {
            ...
            "reason": "Modification outside src/ requires approval"
          }
        ]
      }
    ],
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "Review and summarize completed work"
          }
        ]
      }
    ]
...
When a hook command returns, it should output JSON to stdout:

```json
{
  "continue": true,           // false = stop execution
  "stopReason": null,         // Reason if continue=false
  "suppressOutput": false,    // Hide hook output from user
  "hookSpecificOutput": {
...
class HooksConfiguration:
    ...
    def from_file(cls, path: Path) -> HooksConfiguration:
        ...
        try:
            ...
            logger.warning(f"Failed to parse hooks.json: {e}")
            return cls()
    ...
    def from_dict(cls, data: dict[str, Any]) -> HooksConfiguration:
        ...
        return cls(
            ...
            pre_tool_use=[
                HookMatcher.from_dict(m) for m in hooks.get("PreToolUse", [])
            ],
            post_tool_use=[
                HookMatcher.from_dict(m) for m in hooks.get("PostToolUse", [])
            ],
            stop=[HookMatcher.from_dict(m) for m in hooks.get("Stop", [])],
            subagent_start=[
                HookMatcher.from_dict(m) for m in hooks.get("SubagentStart", [])
            ],
            subagent_stop=[
                HookMatcher.from_dict(m) for m in hooks.get("SubagentStop", [])
            ],
        )
...
class FileBasedHookExecutor:
    ...
    def _create_post_tool_callback(
        self, hooks: list[HookDefinition]
    ):
        ...
        async def callback(
            ...
            for hook in hooks:
                ...
                if hook.type == HookType.LOG:
                    ...
                    logger.info(f"Hook log: {json.dumps(log_entry)}")
    ...
    async def _execute_command_hook(
        ...
        try:
            ...
            logger.warning(f"Hook command timed out: {hook.command}")
            return HookResponse(
                continue_execution=False,
