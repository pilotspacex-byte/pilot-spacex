...
class AnthropicLLMMixin:
    ...
    async def query(
        ...
        payload = {
            "model": self.DEFAULT_MODEL if hasattr(self, 'DEFAULT_MODEL') else "claude-sonnet-4-20250514",
    ...
    async def _get_anthropic_client(
        ...
        if not hasattr(self, '_key_storage'):
            raise AttributeError(
                "AnthropicLLMMixin requires _key_storage. "
                "Ensure agent __init__ stores key_storage."
            )
    ...
    def _extract_caching_config(
        ...
        return CachingConfig(
            ...
            message_flow_cache_threshold=int(model_parameters.pop("prompt_caching_message_flow", 0) or 0),
        )
