...
from pilot_space.ai.agents.sdk_mixin import ClaudeAgentMixin, AgentQueryOptions
...
class IssueEnhancerAgent(SDKBaseAgent[IssueEnhancementInput, IssueEnhancementOutput], ClaudeAgentMixin):
    ...
    SYSTEM_PROMPT = """You are an AI assistant that enhances issue tickets.
Analyze the given issue and provide improvements in JSON format with:
- enhanced_title: Improved, clear title
- enhanced_description: Expanded description with context and structure
- suggested_labels: List of relevant labels with confidence scores
- suggested_priority: Priority suggestion with confidence
"""
    ...
    async def execute(
        ...
        response_text = await self.sdk_execute(prompt, context, options)
