...
class ConversationAgent(StreamingSDKBaseAgent[ConversationInput, ConversationOutput], ClaudeAgentMixin):
    ...
    async def stream(
        ...
        async for chunk in self.sdk_stream_text(prompt, context, options):
            yield chunk
