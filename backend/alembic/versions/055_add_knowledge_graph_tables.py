...
Revises: 054_add_pilot_api_keys_service_role_policy
...
down_revision: str | None = "054_add_pilot_api_keys_service_role_policy"
...
def upgrade() -> None:
    ...
    op.execute(
        text("CREATE INDEX ix_graph_nodes_workspace_type ON graph_nodes (workspace_id, node_type)")
    )
    ...
    op.execute(text("CREATE INDEX ix_graph_nodes_properties ON graph_nodes USING gin (properties)"))
