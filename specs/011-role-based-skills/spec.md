...
5. **Given** a user wants a role not in the predefined list, **When** they click "Custom Role", **Then** they see a text area to describe their responsibilities and focus areas. The role name is auto-generated by AI during skill generation (no manual name input required upfront). The user can edit the suggested name before saving.
...
3. **Given** a user chooses "Describe your expertise" and writes "I'm a QA engineer with 5 years of automation experience. I focus on API testing, performance testing, and CI/CD pipeline reliability. I use behavior-driven development.", **When** the AI generates the skill, **Then** the resulting skill emphasizes API test coverage, performance benchmarks, pipeline stability checks, and BDD-style scenario suggestions — going beyond the generic Tester template. The AI also generates a suggested role name (e.g., "Senior QA Automation Engineer") displayed above the skill preview. The user can edit this name inline before saving.
...
- **FR-003**: System MUST generate a personalized skill description AND a suggested role name using AI based on the user's selected role and self-described experience. For predefined roles, the AI enhances the template name (e.g., "Developer" → "Senior Full-Stack TypeScript Developer"). For custom roles, the AI generates the name entirely from the description. The user can edit the suggested name before saving.
